...
public final class DropSlotLayout {
    ...
    public static int guaranteedSlots(int guaranteed, int weighted, int capacity) {
        ...
        // Math.clamp is Java 21; this branch compiles at 17, which the Forge 1.20.1 toolchain fixes.
        int ceiling = Math.min(guaranteed, capacity - 1);
        return Math.max(1, Math.min(fairShare, ceiling));
    }
