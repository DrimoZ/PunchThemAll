 ...
 * bug. {@link Identifier#tryParse} is used throughout and an unparseable id reports "does not
 * exist"; {@code Identifier.parse} would throw from inside the reload listener and take the
...
public class ItemChecker {
    ...
    private static TagKey<Item> itemTagKey(String itemTag) {
        Identifier id = tryParse(itemTag);
        return id == null ? null : TagKey.create(Registries.ITEM, id);
    }

    /** {@code Identifier.tryParse} rejects malformed ids but still throws on null. */
    static Identifier tryParse(String id) {
