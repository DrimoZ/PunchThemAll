...
public class PtaHand {
    ...
    /**
     * How much to spend on a click whose {@link #shouldConsume()} roll succeeded: a uniform draw
     * from [min, max]. Independent of the chance roll — the two compose, e.g. a 33% chance of
     * spending 3 to 5.
     */
    public int rollConsumeCount() {
        if (isEmpty()) return 0;
        return consumeMin >= consumeMax ? consumeMin : ThreadLocalRandom.current().nextInt(consumeMin, consumeMax + 1);
    }
