...
- Datapacks override each other by pack order. Load conditions (`forge:conditions`) are **not**
  supported on this branch: the loader reads the files itself rather than going through vanilla
  recipe loading, so a conditions block is ignored and the file always loads. The NeoForge 1.21.1
  line does support them.
...
- **Item NBT is matched against a version-stable view** (`Damage`, `Enchantments:[{id,lvl}]`,
  `custom`), so the same `path` / `nbt` expressions work across mod versions. On this branch they are
  also what the item actually stores; the 1.21 line maps data components into the same shape.
...
This branch still reads the legacy (schema 1) format, with a warning; it is deprecated and gains no
new field. Everything below is `schema_version: 2`. If you have files from the old lenient format
